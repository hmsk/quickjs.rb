...
VALUE find_ruby_error(JSContext *ctx, JSValue j_error)
{
  ...
  //
  // Whether it was uncatchable is noted with it: JS_Throw clears that flag and
  // JS_GetException leaves it alone, so putting a lapsed budget's interrupt
  // back without it would hand the guest something it can catch and return
  // from, which is what j_rethrow_the_guests_own guards against by hand.
  ...
  bool was_uncatchable = was_pending && eval_budget_lapsed_now(ctx);
  if (was_pending)
  {
  ...
  if (was_pending && JS_VALUE_GET_PTR(j_trap_threw) == JS_VALUE_GET_PTR(j_was_pending))
  {
    // The same throw is still sitting there, so it is not ours to take.
    JS_Throw(ctx, j_trap_threw);
    if (was_uncatchable)
      JS_SetUncatchableException(ctx, TRUE);
    JS_FreeValue(ctx, j_was_pending);
    return r_error;
  ...
  // displaced. Taking the trap's throw is this function's business; losing an
  // unrelated one that was already travelling is not, and the guard at the top
  // exists to say so.
  if (was_pending)
  {
    JS_Throw(ctx, j_was_pending);
    if (was_uncatchable)
      JS_SetUncatchableException(ctx, TRUE);
  }
  return r_error;
...
// The same question, for the readers that hold a context rather than the data.
...
static bool js_carries_bridged_error(JSContext *ctx, JSValueConst j_val, VMData *data)
{
  ...
  bool bridged = false;
  // FLOAT64 as well as INT, and int64_t rather than int32_t: handles are drawn
  // from a range wider than a tagged int, so QuickJS carries most of them as
  // doubles and a 32-bit read would miss every one of them. The other three
  // readers of this table widened when the draw did.
  int32_t tag = JS_VALUE_GET_NORM_TAG(desc.value);
  if ((desc.flags & JS_PROP_TMASK) == JS_PROP_NORMAL && (tag == JS_TAG_INT || tag == JS_TAG_FLOAT64))
  {
    int64_t object_id = 0;
    JS_ToInt64(ctx, &object_id, desc.value);
    if (object_id > 0)
      bridged = !NIL_P(rb_hash_aref(data->alive_objects, LL2NUM(object_id)));
  }
  JS_FreeValue(ctx, desc.value);
  JS_FreeValue(ctx, desc.getter);
  JS_FreeValue(ctx, desc.setter);
