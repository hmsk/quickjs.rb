...
// Look up a Ruby File from alive_objects by its handle (object_id)
...
static JSValue js_ruby_file_name(JSContext *ctx, JSValueConst _this, int argc, JSValueConst *argv)
{
  ...
  if (NIL_P(r_file)) return JS_UNDEFINED;
...
{
  // argv[0] = handle, argv[1] = start, argv[2] = end, argv[3] = contentType
  VALUE r_file = r_find_alive_rb_file(ctx, argv[0]);
  if (NIL_P(r_file)) return JS_UNDEFINED;
  ...
  // Create new Blob([uint8array], {type: contentType})
