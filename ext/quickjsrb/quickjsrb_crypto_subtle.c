...
// A rejected promise carrying a TypeError. These entry points are specified to
// always return a promise, so a refusal decided before the operation starts
// still has to arrive as a rejection rather than as a throw at the call: a
// guest writing importKey(...).catch(h) would otherwise never reach h.
static JSValue js_rejected_type_error(JSContext *ctx, const char *message)
{
  ...
  JSValue j_thrown = JS_GetException(ctx);
  JSValue j_settled = JS_Call(ctx, resolving_funcs[1], JS_UNDEFINED, 1, (JSValueConst *)&j_thrown);
  JS_FreeValue(ctx, j_thrown);
  JS_FreeValue(ctx, j_settled);
...
static void js_reject_with_ruby_error(JSContext *ctx, JSValueConst *resolving_funcs)
{
  ...
  rb_set_errinfo(Qnil);
  // Read and converted before anything is allocated, which is the ordering
  // j_error_from_ruby_error follows: #message is host code and can raise
  // anything, StringValueCStr raises on a non-String and on an embedded NUL,
  // and a raise once j_err exists would longjmp out of a JSCFunction past it
  // and past the promise capability the caller is holding.
  VALUE r_message = rb_funcall(r_error, rb_intern("message"), 0);
  StringValueCStr(r_message);
...
static JSValue js_subtle_generate_key(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
  ...
  if (NIL_P(r_usages))
    return js_rejected_type_error(ctx, "SubtleCrypto: keyUsages has more entries than a key can have");
