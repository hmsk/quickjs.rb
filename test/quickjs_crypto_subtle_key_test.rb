...
describe "crypto.subtle key management" do
  ...
  describe "when the guest throws the key's own rb_object_id" do
    ...
    # These entry points are specified to always return a promise, so a refusal
    # decided before the operation starts still has to arrive as a rejection:
    # a guest writing importKey(...).catch(h) must reach h.
    it "refuses an oversized keyUsages list as a rejection, not a throw" do
      vm = Quickjs::VM.new(features: [::Quickjs::POLYFILL_CRYPTO])

      outcome = vm.eval_code(<<~JS)
        let o = 'none';
        await crypto.subtle.importKey('raw', new Uint8Array(16), { name: 'HMAC', hash: { name: 'SHA-256' } }, false, { length: 10_000_000 })
          .catch((e) => { o = 'rejected with ' + e.name });
        o
      JS

      _(outcome).must_equal 'rejected with TypeError'
    ensure
      vm&.dispose!
    end
