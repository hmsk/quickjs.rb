  ...
  describe "RubyBasedTimeout" do
    ...
    class ProcessBlocking < QuickjsVmTest
      ...
      test 'ensure Kernel#sleep via a provided function is fine' do
        ...
        assert_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('await rbsleep(0.1);')
        end

        assert_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('async function top () { await new Promise(async resolve => { rbsleep(0.1); resolve(); }); } await top();')
        end
      end

      test 'os sleep messes' do
        refute_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('os.sleep(100);')
        end
      end

      test 'awaiting os.setTimeout messes' do
        refute_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('await new Promise(resolve => os.setTimeout(resolve, 100));')
        end
      end

      test 'awaiting async function which wraps os.setTimeout messes' do
        refute_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('async function top () { await new Promise(resolve => os.setTimeout(resolve, 100)); } await top();')
        end
      end

      test 'awaiting os.sleepAsync messes' do
        refute_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('async function top () { await os.sleepAsync(100); } await top();');
        end
      end
    end
    ...
    class OSLessSetTimeout < QuickjsVmTest
      ...
      test 'awaiting setTimeout does not block other threads' do
        assert_sleep_a_sec_within_thread do
          @vm.eval_code_wrap('await new Promise(resolve => setTimeout(resolve, 100));')
        end
      end
    end
  end
